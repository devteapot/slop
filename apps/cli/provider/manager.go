...
func (m *Manager) Connect(ctx context.Context, address string) error {
	...
	return m.connectWithTransport(ctx, address, transport)
}

// ConnectWithTransport connects using a caller-supplied transport (e.g., bridge relay).
func (m *Manager) ConnectWithTransport(ctx context.Context, address string, transport slop.ClientTransport) error {
	return m.connectWithTransport(ctx, address, transport)
}
