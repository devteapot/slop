...
---
SLOP is transport-agnostic. The protocol defines message semantics (see [Messages](/spec/core/messages)); this document defines how consumers find providers and establish connections.
...
**Origin binding (REQUIRED).** Web discovery is attacker-influenced input: any page can serve a `<meta name="slop">` tag pointing at a WebSocket it does not own — including another local app's loopback provider. Consumers (browser extensions, desktop apps) MUST therefore:

- **Attribute identity from the connection, not the page.** A provider's identity (id, name, capabilities) comes from the `hello` message — or the `/.well-known/slop` descriptor — received over the actual connection. The advertising page's own claims (its title, URL, or meta tag content) MUST NOT be used as the provider's identity.
- **Treat cross-origin meta targets as untrusted.** A meta tag whose WebSocket target is cross-origin relative to the advertising page — a different host, or a loopback address when the page itself is not served from loopback — may be pointing at someone else's provider. Consumers MUST treat such a target as untrusted: they SHOULD require explicit user confirmation before connecting, and MUST NOT silently merge the resulting provider into the page's identity (e.g. listing it under the page's tab, or routing user/AI actions intended for the page to it).

`/.well-known/slop` does not have this problem at the discovery step: it is fetched from the page's own origin, so the descriptor is same-origin by construction. The meta tag is the mechanism that can point anywhere, and it is the one this rule constrains. (A same-origin descriptor can still name a cross-origin transport URL — the identity-attribution rule above applies to the resulting connection either way.)
...
| `async` | Actions may complete asynchronously; `invoke` may return `status: "accepted"`. | Never return `accepted`. Consumers treat any stray `accepted` as `ok` (see [messages.md §invoke result](/spec/core/messages#extended-result-statuses)), but providers SHOULD NOT emit it when the capability is absent. |
| `content_refs` | Nodes may include `content_ref` for out-of-band content. The provider exposes a `read_content` affordance (invoked via the standard `invoke` message) to return inline content for `slop://` references. See [content-references.md](/spec/extensions/content-references). | No `content_ref` fields appear in the tree. An `invoke` targeting a `read_content` affordance is rejected like any other unknown action. |
...
- **WebSocket transports** MUST authenticate every connection that is not bound to `127.0.0.1` / `::1`. The authentication check happens *during* the HTTP upgrade — before the WebSocket is accepted and before any SLOP message is delivered. Reference SDKs expose an `authenticate(request)` hook on every WebSocket handler (TypeScript Node/Bun/Nitro, Go `WebSocketHandlerWithOptions`, Python `serve(..., authenticate=...)` and `SlopMiddleware(..., authenticate=...)`, Rust `websocket::serve_with_options` and `slop_router_with_options`); implementers MUST wire this hook and reject the upgrade (`401`/`403`) when it returns false or throws. SDKs MUST default-deny when no `authenticate` hook is configured — accepting upgrades silently is a protocol violation.
  ...
  - Tokens MUST be compared in constant time and MUST NOT be logged. Servers MUST validate the request `Origin` against an allowlist on every browser-originated upgrade — **including on listeners bound to `127.0.0.1` / `::1`**. Loopback binding does not exempt a server from Origin validation: any web page the user visits can open `ws://127.0.0.1:<port>`, so an unchecked loopback listener is reachable by every site in the user's browser. The only case where the Origin check may be skipped is an upgrade that carries no `Origin` header at all (a non-browser client) arriving on a loopback-bound listener. When no allowlist is configured, implementations MUST default-deny any upgrade that carries an `Origin` header — the reference TypeScript handlers (`attachSlop`, Bun, Nitro) behave this way and respond `403`. `Origin: null` or a wildcard MUST NOT be accepted in production. Development-only origin bypass flags are acceptable only when they default to off and the server logs a clear warning when active.
...
- **Web discovery claims are not provider identity.** A `<meta name="slop">` tag is a claim by the advertising page, not proof of ownership. Consumers MUST attribute a provider's identity from the `hello` message received over the actual connection, and MUST treat a meta tag whose WebSocket target is cross-origin relative to the page (different host, or loopback when the page is not loopback-served) as untrusted — explicit user confirmation before connecting, never a silent merge into the page's identity. See [Web discovery](#web-discovery).
