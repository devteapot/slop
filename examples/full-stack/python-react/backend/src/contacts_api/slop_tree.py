...
from .ui_mount import schedule_mounted_ui_refresh
...
def register_tree(slop: SlopServer) -> None:
    ...
    def _with_ui_refresh(handler: Any) -> Any:
        def wrapped(params: dict[str, Any]) -> Any:
            # App-owned policy: server-side SLOP mutations ask the mounted UI to refetch.
            result = handler(params)
            schedule_mounted_ui_refresh(slop)
            return result
        return wrapped
    ...
    def _make_contact_actions(contact_id: str) -> dict[str, Any]:
        ...
        return {
            "edit": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.edit_contact(cid, **params)),
                "idempotent": True,
                "params": {
                    "name": "string",
                    "email": "string",
            ...
            "delete": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.delete_contact(cid)),
                "dangerous": True,
            },
            "star": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.star(cid)),
                "idempotent": True,
            },
            "unstar": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.unstar(cid)),
                "idempotent": True,
            },
            "add_tag": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.add_tag(cid, params["tag"])),
                "params": {"tag": "string"},
            },
            "remove_tag": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.remove_tag(cid, params["tag"])),
                "params": {"tag": "string"},
            },
            "add_note": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.add_note(cid, params["content"])),
                "params": {"content": "string"},
            },
            "log_activity": {
                "handler": _with_ui_refresh(lambda params, cid=contact_id: state.log_activity(
    ...
    def contacts_node() -> dict[str, Any]:
        ...
        return {
            ...
            "actions": {
                "create": {
                    "handler": _with_ui_refresh(_handle_create),
                    "params": {
                        "name": "string",
                        "email": "string",
                        "phone": "string",
            ...
            "items": [
                {
                    ...
                    "actions": {
                        "rename": {
                            "handler": _with_ui_refresh(lambda params, t=tag: state.rename_tag(t, params["new_name"])),
