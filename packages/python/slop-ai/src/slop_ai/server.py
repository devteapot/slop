    ...
    def emit_event(self, name: str, data: Any = None) -> None:
        ...
        for conn in self._connections:
            try:
                ...
                logger.warning("failed to send event to connection", exc_info=True)
    ...
    def stop(self) -> None:
        ...
        for conn in list(self._connections):
            try:
                ...
                logger.debug("error closing connection during shutdown", exc_info=True)
    ...
    def _rebuild(self) -> None:
        ...
        for path, fn in self._dynamic.items():
            try:
                ...
                logger.warning("error evaluating descriptor at '%s'", path, exc_info=True)
    ...
    def _broadcast_patches(self, ops: list[PatchOp]) -> None:
        for sub in self._subscriptions:
            try:
                ...
                logger.warning("failed to broadcast patches to subscriber %s", sub.id, exc_info=True)
