...
The bridge listens on loopback, but loopback is not a trust boundary — any local process, and any web page open in the user's browser, can attempt a connection to `ws://localhost:9339`. An unauthenticated peer that connects can announce fake providers (`provider-available`) or sit in the relay path, poisoning what the user's AI sees. The bridge therefore enforces the WebSocket rules from [Transport & Discovery § Security considerations](../core/transport.md#security-considerations), specialized for its one legitimate peer:
