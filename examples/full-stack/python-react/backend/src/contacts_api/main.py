...
class _UiMountMiddleware:
    """Intercept provider WebSocket connections before SlopMiddleware."""
    ...
    async def __call__(self, scope: Any, receive: Any, send: Any) -> None:
        if (
            ...
            and scope.get("path") == "/slop"
            and is_provider_connection(scope)
        ):
            mount_path = get_mount_path(scope)
            await handle_provider_websocket(slop, scope, receive, send, mount_path)
            return
        await self.app(scope, receive, send)


app.add_middleware(SlopMiddleware, slop=slop)
app.add_middleware(_UiMountMiddleware)
